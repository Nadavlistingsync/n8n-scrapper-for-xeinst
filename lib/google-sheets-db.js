...
function csvRowToLead(row) {
  ...
  return {
    ...
    ai_recommendation: fields[14] || undefined,
...
async function insertLead(lead) {
  ...
  // Only accept leads with Gmail emails
  if (!isGmailEmail(lead.email)) {
    console.log(`🚫 Skipping non-Gmail email: ${lead.email} (${lead.github_username})`)
    return null
  }
  ...
  console.log(`✅ Added Gmail lead: ${lead.email} (${lead.github_username})`)
  return newLead
}
...
async function getLeadsForEmailCampaign() {
  const leads = await getAllLeads()
  return leads.filter(lead => 
    lead.email && 
    !lead.email_sent && 
...
async function saveLeads(leads) {
  ...
  // Auto-sync to Google Sheets if API is configured
  try {
    const { syncToGoogleSheets } = require('./google-sheets-api')
    const analytics = await getAnalytics()
    await syncToGoogleSheets(leads, analytics)
  } catch (error) {
    // Silently fail if Google Sheets API is not configured
    // This allows the system to work without API setup
  }
}
